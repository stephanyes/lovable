...
class CreateMenuContainer extends React.Component {
    ...
    handleSubmit(e) {
        e.preventDefault()
        let doc = DB.collection('restaurants').doc(this.props.userLogin).collection("menu").doc(this.menuId).collection('categories').doc()
        doc.set(this.state)
    ...
    render() {
        return (
            <div>
                <SidebarContainer />
                <CreateCategoryView submit={this.handleSubmit} inputs={this.handleInputs} />
            </div>
        );
    }
}
