...
class EditCategoryContainer extends React.Component {
    ...
    componentDidMount() {
        let doc = DB.collection('restaurants').doc(this.props.restoId).collection("menu").doc(this.menuId).collection('categories').doc(this.categoryId)
    ...
    render() {
        ...
        return (
            <div>
                <SidebarContainer />
                <EditCategoryView inputs={this.handleInputs} submit={this.handleSubmit} category={this.state} />
            </div>
        )
    }
}
