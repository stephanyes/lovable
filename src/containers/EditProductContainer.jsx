...
class EditProductContainer extends React.Component {
    ...
    componentDidMount() {
        ...
        let doc = DB.collection('restaurants').doc(this.props.restoId).collection("menu").doc(this.menuId).collection('categories').doc(this.categoryId).collection('products').doc(this.productId)
    ...
    render() {
        ...
        return (
            <div>
                <SidebarContainer />
                <EditProductView inputs={this.handleInputs} submit={this.handleSubmit} product={this.state} />
            </div>
        )
    }
}
